 ...
 *
 * Flow:
 * 1. User picks from/to asset and enters amount
 * 2. After 600ms debounce, queries /api/wallet/swap-estimate for the
 *    real exchange rate and DEX path
 * 3. Shows "You'll receive ~X USDC" with slippage-adjusted minimum
 * 4. On submit, calls /api/wallet/swap with the path from the estimate
 */
...
interface SwapPathAsset { code: string; issuer: string | null; }
...
export function SwapModal({ accessToken, onClose, onSuccess }: SwapModalProps) {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        ...
        if (res.ok) {
          ...
          setEstimateError(data.error || 'Could not get estimate');
        }
      } catch {
        setEstimateError('Network error getting estimate');
      } finally {
        setEstimating(false);
      }
    }, 600);
  ...
  async function handleSwap(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.details || data.error || 'Swap failed');
      }
    } catch {
